...
publish of `NetPrints.Generator`). Versioned by MinVer from `v*` git tags like every package (release-and-docs.md §1);
packing and publishing are specified in release-and-docs.md §3 and §7.
...
| Rule | Contract |
|---|---|
| File content | `// <auto-generated>\n//     Generated by NetPrints from <graph file name>. Do not edit.\n// </auto-generated>\n` + `TranslatedClass.Code` with `\r\n` → `\n`, ending with one `\n`. Deterministic: no timestamps, no absolute paths, no NetPrints version (MinVer gives every commit its own version, so a version here would rewrite every committed file on each update; release-and-docs.md §1). |
