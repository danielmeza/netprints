...
**Revision 2026-09-26 (owner decision: no legacy conversion, strict ids, research R21)**: T054 dropped; new
T054a (one-time migration of the repository's legacy files), T054b (strict ids) and T062a (legacy code
removal); T057, T059, T062 and T063 rewritten; done tasks whose legacy code goes later carry a note.
...
- [x] T015 [US1] Id generation (data-model.md §2): `src/NetPrints.Core/Core/Ids.cs` with `IIdGenerator`, `RandomIdGenerator`, `SeededIdGenerator`, `IdGeneration` (`AsyncLocal` scope), `StableIds`; tests `tests/NetPrints.Core.Tests/Core/IdGenerationTests.cs`: 10 000 ids match `^m[0-9a-hjkmnp-tv-z]{6}$` for prefix `m`; two `SeededIdGenerator(42)` give the same sequence; `Use` restores the previous generator on `Dispose`, flows across `await` and does not leak into a parallel task; `StableIds.SeedFor("")` = `unchecked((int)2166136261)` and `SeedFor("a")` = `unchecked((int)0xE40C292C)`; `IsValidDocumentId` rejects `""`, `"a/b"`, `"a b"` *(2026-09-26: `IsValidDocumentId` is replaced by `IdFormat.IsValid` in T054b; `SeedFor` goes with T062a.)*
- [x] T016 [US1] Node identity: `Node.Id`, `Node.DefaultName`, `NodeGraph.AllocateNodeId/FindNode/AssignLegacyNodeIds/PreservedDocumentState` (data-model.md §2) in `src/NetPrints.Core/Graph/Node.cs`, `src/NetPrints.Core/Core/NodeGraph.cs`; `[OnDeserializing]` init; tests `tests/NetPrints.Core.Tests/Core/NodeIdTests.cs`: DF-T18 node part (random ids, retry on a used id with a stub generator, seeded determinism, legacy `n0…nK`, `AssignLegacyNodeIds` throws when ids are set, undo-style remove/re-add keeps the id) *(2026-09-26: `AssignLegacyNodeIds` and its tests are removed by T062a.)*
- [x] T017 [US1] Member identity, graph keys and dirty state: `Id` on `MethodGraph`, `ConstructorGraph`, `Variable` (constructor-assigned, `internal set`); `ClassGraph.Members`, `EnsureUniqueMemberIds`, `AssignLegacyMemberIds`, `IsDirty`/`MarkDirty`/`MarkClean` in `src/NetPrints.Core/Core/ClassGraph.cs`; `src/NetPrints.Core/Core/GraphKeys.cs` (`class`, `<memberId>`, `<variableId>/type|get|set`); `Project.CreateNewClass` sets dirty (T055 adds the method; until then only the flag exists); tests `tests/NetPrints.Core.Tests/Core/GraphKeyTests.cs`: `For`/`Resolve` round trip for every graph of AllNodes, keys unchanged after reordering `Methods`, `AssignLegacyMemberIds` gives the same ids for two loads of the same legacy class, `EnsureUniqueMemberIds` renames only the later duplicate, `For` throws for a detached graph (DF-T22 model part) *(2026-09-26: `AssignLegacyMemberIds` and its test are removed by T062a.)*
...
- [x] T020 Extract `GraphTypeInference.Relax(NodeGraph)` into `src/NetPrints.Core/Graph/GraphTypeInference.cs` from `MethodGraph.OnDeserialized`; the hook's body becomes a call to `GraphTypeInference.Relax(this)` (document-format.md §3.1); golden tests unchanged; DF-T27 (legacy-import half: relaxation still runs on that path) *(2026-09-26: the legacy-import half of DF-T27 is removed by T062a; the hook itself by T063.)*
...
- [x] T040b [US1] `NodeGraph` id→node `Dictionary` index (data-model.md §2): `FindNode` O(1); `AllocateNodeId` no longer retries or searches (`IdGeneration.Current.NewId('n')` — the generator's own uniqueness guarantee); the index is `[IgnoreDataMember]`, built lazily (DataContract skips field initializers, document-format.md §3.1) and kept current via `Nodes.CollectionChanged` (add/remove) plus an explicit `ReindexNode` call wherever a node's `Id` changes after being added (`AssignLegacyNodeIds`, `DocumentMapper`'s document-id overwrite and duplicate-id repair) *(2026-09-26: the `AssignLegacyNodeIds` call site goes with T062a.)*
...
- [x] T041 [US1] JSON Schema (document-format.md §6): `Json/NetPrintsJsonSchema.cs`; generate and commit `schemas/netpc.v1.schema.json`; DF-T24 in `tests/NetPrints.Core.Tests/Serialization/SchemaTests.cs` (the test locates the repository root with `SampleProjectFactory.FindRepositoryRoot()`). Inspect the exporter's polymorphism output first and record what `TransformSchemaNode` had to fix in research.md R17 (open item K14) *(2026-09-26, R21: the id `pattern`s this task deliberately left out are added by T054b.)*
- [x] T042 [US1] Round trips DF-T02, DF-T03, DF-T05 (graph level) and the merge test DF-T23 in `…/Serialization/RoundTripTests.cs` and `…/Serialization/MergeTests.cs` (`git merge-file -p` via `System.Diagnostics.Process`, temp directory); switch `GoldenCSharpTests` to the new importer (DF-T01) *(2026-09-26: DF-T01…T03 read the migrated JSON since T054a.)*
- [x] T043 [US1] **Checkpoint C**; commit

## Phase 4: User Story 1b — build pipeline (P1) (sub-phase D)

- [x] T044 [US1] `src/NetPrints.Generator/GraphCodeGenerator.cs` (`GenerateAsync`, `RenderFile`; project-system.md §3), `GenerateRequestFile.cs`, `Program.cs` (`generate` command, exit codes, canonical error lines with member-id graph keys) + unit tests PS-T06 (header, `\n`, determinism) in `tests/NetPrints.Core.Tests/Projects/GraphCodeGeneratorTests.cs` *(2026-09-26: its tests build their input from the legacy fixture until T054a switches them to JSON.)*
...
## Phase 5: User Stories 1c + 2 — project system, migration, references (P1) (sub-phase E)
...
- [x] T052 [US1] Profiles: `src/NetPrints.Core/Profiles/IProjectProfile.cs`, `ClassTemplate.cs`, `DefaultProjectProfile.cs` (extension-points.md §5, `ProjectTemplate` = project-system.md §1)
- [x] T053 [US2] `src/NetPrints.Workspace/MsBuildProjectSystem.cs`: `LoadAsync` (restore check, evaluation, MSBuildWorkspace, docs paths; logs 4001–4003), `ApplyAsync` (`ProjectRootElement`, atomic save), `CreateAsync` (`.csproj` + `.gitattributes`), `BuildAsync` (log 4004), `GetRunCommand`; tests PS-T07, PS-T08, PS-T09, PS-T11 and the `CreateAsync` part of PS-T15 in `tests/NetPrints.Core.Tests/Projects/MsBuildProjectSystemTests.cs` (local test package built into a temp feed for PS-T08)
- T054 — *dropped 2026-09-26 (owner decision, research R21)*: no `ProjectConverter`, no generator `convert` command, no PS-T12, no conversion part of PS-T15. Replaced by T054a
- [x] T054a [US1] One-time migration of the repository's legacy files (research R21; before anything that needs HelloWorld as a `.csproj`). Throwaway converter `tests/NetPrints.Core.Tests/Characterization/LegacyMigration.cs`: a test that does nothing unless `NETPRINTS_MIGRATE_LEGACY=1`, built on the existing code: for `Fixtures/Legacy/AllNodes/AllNodes.Everything.netpc` and `Fixtures/Legacy/HelloWorld/HelloWorld.Program.netpc`, read with `LegacyXmlDocumentFormat`, rewrite every positional node id (`n<index>`) to an id from `SeededIdGenerator(StableIds.SeedFor(<class full name> + "/nodes"))` in graph order, consistently in `nodes`, `connections` and `layout` (e.g. load with `DocumentMapper.FromDocument`, set each `Node.Id` with `ReindexNode`, map back), and write with `JsonDocumentFormat`; member ids are kept (`AssignLegacyMemberIds` already gives `IdFormat` ids); every written id matches `IdFormat.Pattern`. Run it once and commit the output: `tests/NetPrints.Core.Tests/Fixtures/AllNodes/AllNodes.Everything.netpc.json`, `tests/NetPrints.Core.Tests/Fixtures/HelloWorld/HelloWorld.Program.netpc.json` and the same HelloWorld graph as `samples/HelloWorld/HelloWorld.Program.netpc.json` (the legacy sample files stay until T057). Write the project files by hand from the `.netpp` settings (project-system.md §1, §2.1): `samples/HelloWorld/HelloWorld.csproj` and `tests/NetPrints.Core.Tests/Fixtures/HelloWorld/HelloWorld.csproj` (`Exe`, `RootNamespace` `HelloWorld`, the conditional `NetPrints.Sdk` `PackageReference`), `samples/HelloWorld/.gitattributes` (§1.1), `tests/NetPrints.Core.Tests/Fixtures/AllNodes/AllNodes.csproj` (`Library`, `RootNamespace` `AllNodes`, `<None Include="ExternalSources/**/*.cs" NetPrintsSourceDirectory="true" />`; the `.NETFramework` references and the reference to the never-present `ExternalLibrary.dll` are dropped). Add `*.netpc.json text eol=lf` and `*.netpc.g.cs text eol=lf` to the root `.gitattributes` (no `linguist-generated`: generated diffs stay visible in PRs). In `tests/NetPrints.Core.Tests/NetPrints.Core.Tests.csproj`, copy the new fixture folders as test content and keep `Fixtures/**/*.cs` out of `Compile`. Switch every test and golden to the JSON files: `GoldenCSharpTests` (DF-T01 through `JsonDocumentFormat` + `DocumentMapper`; rename `LegacyFixtures`), `RoundTripTests` (DF-T02 as rewritten in document-format.md §7, DF-T03 on the committed JSON), `GraphCodeGeneratorTests`. Proof, in the commit message: the suite passes without `NETPRINTS_UPDATE_SNAPSHOTS`, `git diff --exit-code tests/NetPrints.Core.Tests/Fixtures/Golden/` is empty (a golden diff means the migration is wrong: fix the converter, never the golden), and both migrated classes load with no `DocumentIssue`. The converter, the legacy fixtures and the legacy-only tests stay until T062a
- [x] T054b [US1] Strict ids (research R21, document-format.md §1.4.1, §2.6, §6; after T054a, whose files already comply): `IdFormat.PatternFor(char)` and `IdFormat.IsValid(string?, char)` in `src/NetPrints.Core/Core/Ids.cs` (data-model.md §2) replace `StableIds.IsValidDocumentId` (`IdGenerationTests`: `IsValid` accepts a generated id for its own prefix only and rejects `n0`, `start`, upper case, 12 and 14 digits, `null`); `DocumentMapper.FromDocument` replaces a present node or member id that does not match with a fresh one, rewrites the connection endpoints and `layout` keys that name it, and reports `DocumentIssue.InvalidIdReassigned` (`NPD009`, Warning; update the `NPD001`–`NPD007` range in the `DocumentIssue.cs` docs), before duplicate repair; `ValidateMemberIdShapes` keeps only the missing-id check. `Json/NetPrintsJsonSchema.cs` adds the id patterns built from `IdFormat`; regenerate `schemas/netpc.v1.schema.json` with `NETPRINTS_UPDATE_SNAPSHOTS=1` (the diff adds only `pattern`/`propertyNames`) and record in research.md R17's T041 findings anything `TransformSchemaNode` needed for them; DF-T24 asserts them. DF-T28 in `tests/NetPrints.Core.Tests/Serialization/StrictIdTests.cs`. Give every handcrafted document the mapper reads `IdFormat`-shaped ids: `n0`/`n1`/`m000001`-style ids in `DocumentMapperTests`, `JsonDocumentFormatTests`, `BuiltInConverterTests`, and DF-T23's `nb00000`…`nz00000` in `MergeTests` (14-character ids with the same ordinal order, so the connection and layout lines keep their gaps); `CanonicalJsonTests` and `NodeListConverterTests` do not map documents and may keep short ids
- [x] T055 [US1] Project model from snapshot (data-model.md §5): add `Project.FromSnapshot`, `Snapshot`, `GetGraphFilePath`, `CreateNewClass(profile)` (class starts dirty), `LastDiagnostics` next to the old members (which stay until T063 so the solution keeps building while the editor and CLI move over)
...
- [ ] T057 [US1] Switch `samples/HelloWorld` over (FR-010; T054a added its `.csproj`, graph and `.gitattributes`): build it and commit `HelloWorld.Program.netpc.g.cs`; delete `samples/HelloWorld/HelloWorld.netpp` and `HelloWorld.Program.netpc`; replace `SampleProjectFactory` output with this layout (no `.netpp`); update sample paths in `tests/NetPrints.Core.Tests/Samples/HelloWorldSampleTests.cs` (load through `ProjectPersistence`; compile/run through `IProjectSystem`) and README (FR-010); editor tests that still open a `.netpp` through `Project.LoadFromPath` (`tests/NetPrints.Editor.Tests/TestPaths.cs`, `tests/NetPrints.Editor.UITests/Hosting/TestDoubles.cs`, `tests/NetPrints.Testing.Ui/Scenarios/SmokeScenarios.cs`, `tests/NetPrints.Desktop.E2ETests/Scenarios/X11SmokeTests.cs`) and the "CLI sample compile and run" step of `.github/workflows/ci.yml` point at `tests/NetPrints.Core.Tests/Fixtures/Legacy/HelloWorld/HelloWorld.netpp` until T059/T062; end-to-end DF-T01/SC-001 test in `tests/NetPrints.Core.Tests/Samples/MigratedFixtureBuildTests.cs`: build and run a temp copy of `samples/HelloWorld` (`LocalSdkLayout`), run `GraphCodeGenerator.GenerateAsync` on the AllNodes fixture, and compare every `.g.cs` body with its golden; DF-T26 in `tests/NetPrints.Core.Tests/Samples/CommittedSampleTests.cs` (canonical graphs and up-to-date `.netpc.g.cs`; the failure message names the file and says to run the generator or `NETPRINTS_UPDATE_SNAPSHOTS=1`)
...
- [ ] T059 [US2] Editor switch-over: `EditorContext` gains `Projects`, `Persistence`; `MsBuildRegistration` in Desktop `Main` (`src/NetPrints.Desktop/Program.cs`; `EditorHostServices.MsBuildAvailable`, `NoSdkProjectSystem` for PS-T13); `ReflectionHost` from `Project.Snapshot` (logs 1010–1013; the catalog/`IExtensionHost` parameter is added in T076, until then the provider is the live one only; the mapper uses `NodeDocumentConverterRegistry.BuiltIn` until T076); `MainEditorVM` create/open (`*.csproj` only; any other path, e.g. an old `.netpp`, → message, nothing written, R21)/save/add-existing (`*.netpc.json` only) via persistence; Settings pane binary type via `ApplyAsync(SetOutputType)`, Output-flags chooser removed; `ReferenceListVM` on `DeclaredReferences` + `ProjectEdit`s; `src/NetPrints.Editor/Hosting/FileFilter.cs` (`ProjectFiles` `["*.csproj"]`, `ClassFiles` `["*.netpc.json"]`); ED-T13 and PS-T13 in `tests/NetPrints.Editor.Tests/Main/MainEditorVMTests.cs` (replace the existing `.netpp` assertions); move the editor test paths of T057 to `samples/HelloWorld/HelloWorld.csproj`; reference VM tests updated
...
- [ ] T062 [US2] CLI `src/NetPrints.Cli/Program.cs` per editor-services.md §4 (build/run `.csproj`; a `.netpp` or any other file → message and the bad-arguments exit code, no conversion); change the "CLI sample compile and run" step of `.github/workflows/ci.yml` to `-p samples/HelloWorld/HelloWorld.csproj -r`
- [ ] T062a [US1] Delete the legacy code (research R21; after T059 and T062, before T063): `src/NetPrints.Serialization/Legacy/` (`LegacyXmlDocumentFormat`, `LegacyProject` and the four reference copies), `NodeGraph.AssignLegacyNodeIds`, `ClassGraph.AssignLegacyMemberIds`, `StableIds.SeedFor` (no caller left) and their mentions in doc comments (`Node.cs`, `ConstructorGraph.cs`, `MethodGraph.cs`, `Variable.cs`, `IDocumentMapper.cs`, `DocumentMapper.cs`); drop `ProjectFiles.EnsureGitAttributesAsync`'s return value if nothing uses it (project-system.md §1.1). Tests: delete the T054a converter, `tests/NetPrints.Core.Tests/Fixtures/Legacy/**`, `LegacyProjectTests`, `LegacyXmlDocumentFormatTests`, `AllNodesFixtureRegenerationTests` (it compared the factory with the legacy XML; `AllNodesFixtureFactory` stays as an in-memory model builder), the legacy cases of `NodeIdTests`, `GraphKeyTests` and `IdGenerationTests` (`SeedFor`), and the DF-T27 legacy-import half (T020); rename the fake `legacy-xml` format in `DocumentFormatRegistryTests` (DF-T16 keeps its longest-suffix case). Accept: `git grep -nE 'LegacyXml|AssignLegacy|Serialization\.Legacy|SeedFor|Fixtures/Legacy' -- src tests .github` is empty; goldens and DF-T01 unchanged
- [ ] T063 [US2] Delete the old model/persistence APIs now that nothing calls them: `Project.CreateNew/LoadFromPath/Save/SaveClassInProjectDirectory/AddExistingClass/CompileProject/RunProject/GetRunCommand/References/ClassPaths/CompilationOutput/LastCompileErrors`, `src/NetPrints.Core/Serialization/SerializationHelper.cs`, `src/NetPrints.Core/Core/ReferenceAssemblyResolver.cs`, `src/NetPrints.Core/Compilation/CodeCompiler.cs`, `ICodeCompiler.cs`, and Core's legacy reference classes (`AssemblyReference`, `FrameworkAssemblyReference`, `SourceDirectoryReference`, `CompilationReference`, `ICompilationReference` in `src/NetPrints.Core/Core/`), with their `.gitattributes` lines; delete `tests/NetPrints.Core.Tests/Core/ReferenceAssemblyResolverTests.cs` (superseded by PS-T07) and rewrite `tests/NetPrints.Core.Tests/Compilation/DeterministicCompileTests.cs` on the build path (a temp copy of the HelloWorld sample plus 8 classes from `CreateNewClass`, saved with `ProjectPersistence.SaveAsync`; `IProjectSystem.BuildAsync`, record the output assembly and every `.netpc.g.cs`, delete `bin/` and `obj/`, build again, assert identical bytes); update `tests/NetPrints.Editor.UITests/Graph/EditFlowTests.cs` (reload through `ProjectPersistence.LoadAsync`); the golden tests already read the migrated JSON (T054a). The model's DataContract persistence goes too (research R21): the `[DataContract]`, `[DataMember]`, `[KnownType]` and `[IgnoreDataMember]` attributes in `src/NetPrints.Core` (including `ModelObject`), the `[OnDeserializing]` initializers and the `[OnDeserialized]` hooks of document-format.md §3.1 (`Node`, `MethodGraph` — `GraphTypeInference.Relax` stays called from `DocumentMapper` —, `Variable`, `Project.FixDefaults`); `NodeGraph`'s id index may then be built eagerly; `AllNodesFixtureFactory` and `SampleProjectFactory` use only the remaining `Project` API; the T005 notification map and every golden stay unchanged; accept: `git grep -nE 'DataContract|DataMember|KnownType|OnDeserializ|\.netpp' -- src` is empty. Headless test: `WriteLine` node tooltip contains the summary (D5, SC-003) in `tests/NetPrints.Editor.UITests/Graph/NodeTooltipTests.cs`; grep test: no `ProgramFilesX86` under `src/`. Also remove the four temporary XML-documentation exemption sections in the root `.editorconfig` (`[src/NetPrints.Core/Serialization/SerializationHelper.cs]`, `[src/NetPrints.Core/Compilation/CodeCompiler.cs]`, `[src/NetPrints.Core/Compilation/ICodeCompiler.cs]`, `[src/NetPrints.Core/Core/ReferenceAssemblyResolver.cs]`) now that the files are gone
...
- Hard edges: T004/T005 before T007; T015 before T016, T017; T016–T019 before T035; T025 before T026, T027; T027 before T037, T041; T030 before T031–T034; T044 before T045–T048; T046 before T047, T057, T081; T050 before T053, T056; T052 before T053; T053 before T054a; T054a before T054b, T057; T055 before T056, T059; T059 before T060; T059 and T062 before T062a; T062a before T063; T059–T062 before T063; T065 before T067, T074, T079; T068 before T075; T089 before T090, T096; T041, T057, T062, T063 before L; T109 before T110; T110 before T111; T111 before T112, T113, T114; T112 before T113; T114 before T115; T116 before T117; T117 before T118, T120, T121; T104 before T117 (its pages are in the site).
...
MVP = A–E: the roadmap "done when" as revised by R21 (the sample migrated to `.csproj` + JSON graphs in the VCS-friendly format,
...
| Requirement | Tasks |
|---|---|
| FR-043 Snowflake, seedable node ids; strict id shape on read and in the schema (R21) | T015, T016, T040a, T054b |
...
| FR-050 `.gitattributes`; committed-file checks in CI | T050, T053, T054a, T057 |
| SC-009 merge and reorder behaviour | T017, T035, T042 |
| SC-010 committed schema, graphs, `.netpc.g.cs` up to date | T041, T054b, T057 |

## Requirement coverage (legacy removal, revision 2026-09-26)

| Requirement | Tasks |
|---|---|
| FR-007 no legacy reading or conversion; `.netpp` rejected; no legacy code left | T054a, T059, T062, T062a, T063 |
| FR-008 identical C# across the one-time migration (goldens unchanged) | T004, T054a, T057 |
| FR-010 sample as `.csproj` with JSON graph and committed `.netpc.g.cs`; legacy files deleted | T054a, T057, T062a |
| SC-001 fixtures' C# byte-identical from JSON; HelloWorld builds | T054a, T057 |
...
## Test-ID map (revisions 2026-09-25 and 2026-09-26)

| Test ID | Status | Now in |
|---|---|---|
| DF-T01 | kept (reads the migrated JSON since 2026-09-26) | T004 (baseline), T042, T054a, T057 |
| DF-T02, T03, T05 | kept (T03 adds re-parse; T05 is now "exactly one line"); T02 changed 2026-09-26 (JSON fixture → save → load → golden, no legacy step) | T042, T054a |
...
| DF-T16 | kept (fake second format renamed 2026-09-26) | T039, T062a |
| DF-T17 | kept (+ canonical form of the extension node) | T073 |
| DF-T18 | changed (random seeded ids, member ids); legacy-import part removed 2026-09-26 | T016, T017, T040a; legacy cases deleted in T062a |
| DF-T19…T26 | new (pins by name, auto-placement, tolerant read, member ids, merge, schema, default names, committed samples); T22 and T24 changed 2026-09-26 (malformed member id → DF-T28; id patterns) | T018/T035, T019/T035, T026/T037, T017/T035, T042 (T23 ids reshaped in T054b), T041/T054b, T035, T057 |
| DF-T27 | new (§3.1: post-load type-pin wiring and relaxation); JSON path only since 2026-09-26 | T035; the T020 legacy-import half is deleted in T062a |
| DF-T28 | new 2026-09-26 (strict ids: invalid shape → fresh id, references follow, `NPD009`) | T054b |
| RC-T01…T05 | retired (custom resolver removed) → PS-T07, PS-T08 (PS-T12 retired 2026-09-26) | T053 |
| RC-T06, T08, T09, T10 | kept | T089, T091, T058, T090 |
| RC-T07 | retired → PS-T11 | T053 |
| PS-T01…T11, T13, T14 | new | T044–T048, T051, T053, T059, T074 |
| PS-T12 | retired 2026-09-26 (no legacy conversion, R21) | — (was T054) |
| PS-T15 | new (`.gitattributes`); conversion cases removed 2026-09-26 | T050, T053 |
| EX-T01…T12 | kept (EX-T07 reworded for `CreateAsync`) | T065–T076 |
| EX-T13 | changed (trust flow) | T075 |
| ED-T01…T14 | kept (ED-T04, ED-T13 reworded for builds and edited-only saves; ED-T13's conversion flow replaced by "`.netpp` rejected" 2026-09-26) | T022, T059, T075, T083, T088, T092, T096–T102 |
