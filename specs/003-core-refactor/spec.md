...
identical C# (the committed `.netpc.g.cs` is up to date).
...
2. **Given** a converted project, **When** it is saved again without changes, **Then** no file is written and the graph and project files stay byte-identical; **When** only one node is moved, **Then** only its line in the layout section changes.
...
- Two members or nodes end up with the same id after a manual merge: the file is rejected with a message naming the id; nothing is overwritten.
...
- **FR-043**: Node ids MUST be random, unique within their graph and never change once assigned; tests MUST be able to seed the id generator; nodes imported from legacy files MUST get deterministic ids from graph order.
...
- **FR-050**: New and converted projects MUST get a `.gitattributes` that forces LF line endings for graph and generated files; a test that runs in CI MUST fail when a committed `.netpc.g.cs` does not match its graph or a committed graph file is not canonical.
...
- Node ids of imported legacy nodes are derived deterministically from graph order (`n0`, `n1`, …); member ids of imported classes come from a generator seeded with the class name; new nodes and members get random ids.
