...
| `SuggestionListVM` | suggestions for a graph/pin | `SearchText`, `Items` (DynamicData-filtered `ReadOnlyObservableCollection<SuggestionItem>`), `Position` | `Select(SuggestionItem)` → node creation / dialogs via `IEditorDialogs` | calls `NodeGraphVM.AddNode(AddNodeRequest)` |
...
| `GetSetChooserVM` (new) | `VariableSpecifier` + position | `CanGet`, `CanSet`, `IsOpen` | `Get`, `Set` | calls `NodeGraphVM.AddNode(AddNodeRequest)` |
