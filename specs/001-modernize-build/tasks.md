...
- [X] T064 [US2] Add the headless render test `NetPrints.Editor.Tests/Ui/GraphRenderTests.cs`. Open the sample `Main` graph in a `GraphEditorView` inside a headless `Window`. Assert that every `NodeVM` has a realized `ItemContainer` at its `Location`, that connections are realized, and that `CaptureRenderedFrame()` is not null (PAR-38..45 smoke)
