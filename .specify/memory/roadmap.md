...
and therefore requires an installed .NET SDK. Legacy `.netpp`/XML files are import-only and never
modified.
...
caching stays in P8.
Done when: the old sample imports without modifying the legacy file, saves as `.netpc.json` in an
SDK-style project, and `dotnet build` generates identical C# (the committed `.netpc.g.cs` is up to date).
